...
#include "samplv1widget_presets.h"
...
//----------------------------------------------------------------------------
// samplv1widget_presets::ItemDelegate -- Custom (tree) list item delegate.

class samplv1widget_presets::ItemDelegate : public QItemDelegate
{
public:
	// ctor.
...
// ctor.
samplv1widget_presets::ItemDelegate::ItemDelegate (
...
// QItemDelegate interface...
QSize samplv1widget_presets::ItemDelegate::sizeHint (
...
QWidget *samplv1widget_presets::ItemDelegate::createEditor ( QWidget *pParent,
	const QStyleOptionViewItem& /*option*/, const QModelIndex& index ) const
{
	...
	case 0: // Text.
	{
		...
		samplv1widget_presets *pWidget
			= qobject_cast<samplv1widget_presets *>(parent());
		if (pWidget)
...
#ifdef CONFIG_DEBUG
	qDebug("samplv1widget_presets::ItemDelegate::createEditor(%p, %d, %d) = %p",
...
void samplv1widget_presets::ItemDelegate::setEditorData (
	QWidget *pEditor, const QModelIndex& index ) const
{
#ifdef CONFIG_DEBUG
	qDebug("samplv1widget_presets::ItemDelegate::setEditorData(%p, %d, %d)",
...
void samplv1widget_presets::ItemDelegate::setModelData ( QWidget *pEditor,
	QAbstractItemModel *pModel,	const QModelIndex& index ) const
{
#ifdef CONFIG_DEBUG
	qDebug("samplv1widget_presets::ItemDelegate::setModelData(%p, %d, %d)",
		pEditor, index.row(), index.column());
#endif
...
//----------------------------------------------------------------------------
// samplv1widget_presets -- UI wrapper form.

// ctor.
samplv1widget_presets::samplv1widget_presets ( QWidget *pParent )
	: QTreeWidget(pParent)
{
...
// dtor.
samplv1widget_presets::~samplv1widget_presets (void)
{
...
// utilities.
void samplv1widget_presets::loadPresets ( samplv1_presets *pPresets )
{
	QTreeWidget::clear();
	...
	while (bank_iter.hasNext()) {
		...
		samplv1_presets::Bank *pBank = pPresets->find_bank(sBank);
		if (pBank == nullptr)
			continue;
...
void samplv1widget_presets::savePresets ( samplv1_presets *pPresets )
{
	...
	for (int iItem = 0 ; iItem < iItemCount; ++iItem) {
		...
		//	const int iBankData = pBankItem->data(0, Qt::UserRole).toInt();
			samplv1_presets::Bank *pBank = pPresets->add_bank(sBank);
			...
			for (int iChild = 0 ; iChild < iChildCount; ++iChild) {
				...
				samplv1_presets::Preset *pPreset
...
void samplv1widget_presets::loadPresetsCurrent ( samplv1_presets *pPresets )
{
	QString sBank;
	samplv1_presets::Bank *pBank
...
void samplv1widget_presets::savePresetsCurrent ( samplv1_presets *pPresets )
{
...
QString samplv1widget_presets::currentPreset (void) const
{
	QString sPreset;
...
// slots.
void samplv1widget_presets::addBankItem (void)
{
...
void samplv1widget_presets::addPresetItem (void)
{
	QTreeWidget::setFocus();
...
// factory methods.
QTreeWidgetItem *samplv1widget_presets::newBankItem (void)
{
	int iBank = 0;
	const int iItemCount = QTreeWidget::topLevelItemCount();
	for (int iItem = 0; iItem < iItemCount; ++iItem) {
...
QTreeWidgetItem *samplv1widget_presets::newPresetItem (void)
{
...
// file prompts.
//
QStringList samplv1widget_presets::openPresetFiles (void)
{
	QStringList files;
...
QString samplv1widget_presets::savePresetFile ( const QString& sPreset )
{
...
// config stuff.
//
void samplv1widget_presets::setPresetDir ( const QString& sPresetDir )
{
	m_sPresetDir = sPresetDir;
}

const QString& samplv1widget_presets::presetDir (void) const
{
	return m_sPresetDir;
}


void samplv1widget_presets::setPresetExt ( const QString& sPresetExt )
{
	m_sPresetExt = sPresetExt;
}

const QString& samplv1widget_presets::presetExt (void) const
{
	return m_sPresetExt;
}


void samplv1widget_presets::setDontUseNativeDialogs ( bool bDontUseNativeDialogs )
{
	m_bDontUseNativeDialogs = bDontUseNativeDialogs;
}

bool samplv1widget_presets::dontUseNativeDialogs (void) const
{
...
QValidator *samplv1widget_presets::presetsValidator (void) const
{
...
// dirty flag.
void samplv1widget_presets::setDirtyPresets ( bool bDirtyPresets )
{
	if (bDirtyPresets)
		++m_iDirtyPresets;
	else
...
bool samplv1widget_presets::isDirtyPresets (void) const
{
...
// private slots.
//
void samplv1widget_presets::itemChangedSlot ( QTreeWidgetItem *pItem, int )
{
...
void samplv1widget_presets::itemExpandedSlot ( QTreeWidgetItem *pItem )
{
	if (isBankItem(pItem))
		pItem->setIcon(0, QIcon(":/images/presetBankOpen.png"));
}

void samplv1widget_presets::itemCollapsedSlot ( QTreeWidgetItem *pItem )
{
	if (isBankItem(pItem))
...
void samplv1widget_presets::setPresetItem ( const QString& sPreset )
{
...
QTreeWidgetItem *samplv1widget_presets::presetItem ( const QString& sPreset ) const
{
	return nameItem(sPreset, PresetItem);
}

bool samplv1widget_presets::isPresetItem ( QTreeWidgetItem *pItem ) const
{
	return (pItem->type() == PresetItem);
}


void samplv1widget_presets::setBankItem ( const QString& sBank, int iPreset )
{
...
QTreeWidgetItem *samplv1widget_presets::bankItem ( const QString& sBank ) const
{
	return nameItem(sBank, BankItem);
}

bool samplv1widget_presets::isBankItem ( QTreeWidgetItem *pItem ) const
{
	return (pItem->type() == BankItem);
}


QTreeWidgetItem *samplv1widget_presets::nameItem (
...
QString samplv1widget_presets::nameRenum (
	const QString& sName, int iType ) const
{
...
QString samplv1widget_presets::bankRenum ( const QString& sBank ) const
{
	return nameRenum(sBank, BankItem);
}


QString samplv1widget_presets::presetRenum ( const QString& sPreset ) const
{
...
// Drag-n-drop stuff.
//
void samplv1widget_presets::mousePressEvent ( QMouseEvent *pMouseEvent )
{
...
void samplv1widget_presets::mouseMoveEvent ( QMouseEvent *pMouseEvent )
{
...
void samplv1widget_presets::mouseReleaseEvent ( QMouseEvent *pMouseEvent )
{
...
void samplv1widget_presets::dragEnterEvent ( QDragEnterEvent *pDragEnterEvent )
{
...
void samplv1widget_presets::dragMoveEvent ( QDragMoveEvent *pDragMoveEvent )
{
...
void samplv1widget_presets::dragLeaveEvent ( QDragLeaveEvent */*pDragLeaveEvent*/ )
{
	if (m_pRubberBand)
...
void samplv1widget_presets::dropEvent ( QDropEvent *pDropEvent )
{
	const QPoint& pos
	#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
...
bool samplv1widget_presets::canDropEvent ( QDropEvent *pDropEvent )
{
...
QTreeWidgetItem *samplv1widget_presets::dragDropItem ( const QPoint& pos )
{
...
void samplv1widget_presets::dropItem (
...
void samplv1widget_presets::moveRubberBand (
	QTreeWidgetItem *pDropItem, bool bOutdent )
{
	// We must be dragging something...
	QTreeWidgetItem *pDragItem = m_pDragItem;
...
void samplv1widget_presets::ensureVisibleItem ( QTreeWidgetItem *pItem )
{
...
void samplv1widget_presets::autoExpandTimeoutSlot (void)
{
	m_pAutoExpandTimer->stop();
...
// ctor.
samplv1widget_presets::ComboBox::ComboBox ( QWidget *pParent )
	: QComboBox(pParent),
	m_pPresetsView(new samplv1widget_presets(pParent))
{
...
samplv1widget_presets *samplv1widget_presets::ComboBox::presetsView (void) const
{
	return m_pPresetsView;
}


void samplv1widget_presets::ComboBox::setCurrentPreset ( const QString& sPreset )
{
...
QString samplv1widget_presets::ComboBox::currentPreset (void) const
{
	return QComboBox::currentText();
}


void samplv1widget_presets::ComboBox::resetCurrentPreset (void)
{
...
// end of samplv1widget_presets.cpp
