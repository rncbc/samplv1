...
static const char *g_pszDefName = QT_TRANSLATE_NOOP("samplv1widget_config", "(default)");
...
samplv1widget_config::samplv1widget_config (
	samplv1widget *pParent, samplv1_ui *pSamplUi )
	: QDialog(pParent), p_ui(new Ui::samplv1widget_config), m_ui(*p_ui),
		m_pSamplUi(pSamplUi)
{
	// Setup UI struct...
	m_ui.setupUi(this);
...
void samplv1widget_config::controlsContextMenuRequested ( const QPoint& pos )
{
	...
	bool bEnabled = (m_pSamplUi && m_pSamplUi->controls() != nullptr);
...
void samplv1widget_config::presetsAddBankItem (void)
{
...
void samplv1widget_config::presetsAddItem (void)
{
	samplv1_config *pConfig = samplv1_config::getInstance();
	if (pConfig) {
...
void samplv1widget_config::presetsRenameItem (void)
{
...
void samplv1widget_config::presetsRemoveItem (void)
{
	QTreeWidgetItem *pItem = m_ui.PresetsTreeWidget->currentItem();
	if (pItem)
		delete pItem;
...
void samplv1widget_config::presetsImportItems (void)
{
	samplv1_config *pConfig = samplv1_config::getInstance();
	if (pConfig == nullptr)
		return;
...
void samplv1widget_config::presetsExportItems (void)
{
	samplv1_config *pConfig = samplv1_config::getInstance();
	if (pConfig == nullptr)
		return;
...
void samplv1widget_config::programsContextMenuRequested ( const QPoint& pos )
{
	...
	bool bEnabled = (m_pSamplUi && m_pSamplUi->programs() != nullptr);
...
// programs janitor slots.
void samplv1widget_config::presetsCurrentChanged (void)
{
	stabilize();
}


void samplv1widget_config::presetsContextMenuRequested ( const QPoint& pos )
{
	...
	action = menu.addAction(QIcon(":/images/samplv1_preset.png"),
...
void samplv1widget_config::presetsChanged (void)
{
	++m_iDirtyPresets;
...
void samplv1widget_config::presetsActivated (void)
{
...
void samplv1widget_config::tuningScaleFileClicked (void)
{
	samplv1_config *pConfig = samplv1_config::getInstance();
	if (pConfig == nullptr)
		return;
...
void samplv1widget_config::tuningKeyMapFileClicked (void)
{
	samplv1_config *pConfig = samplv1_config::getInstance();
	if (pConfig == nullptr)
		return;
...
void samplv1widget_config::tuningChanged (void)
{
	++m_iDirtyTuning;
...
void samplv1widget_config::stabilize (void)
{
	...
	bEnabled = (m_pSamplUi && m_pSamplUi->programs() != nullptr && m_bPresets);
...
void samplv1widget_config::loadComboBoxHistory ( QComboBox *pComboBox )
{
	samplv1_config *pConfig = samplv1_config::getInstance();
	if (pConfig == nullptr)
		return;
...
void samplv1widget_config::saveComboBoxHistory ( QComboBox *pComboBox )
{
	samplv1_config *pConfig = samplv1_config::getInstance();
	if (pConfig == nullptr)
		return;
...
bool samplv1widget_config::setComboBoxCurrentItem (
...
QString samplv1widget_config::comboBoxCurrentItem ( QComboBox *pComboBox )
{
	QString sData;
