...
class samplv1widget_programs::ItemDelegate : public QStyledItemDelegate
{
public:
...
// painting
//
void samplv1widget_programs::ItemDelegate::paint ( QPainter *pPainter,
	const QStyleOptionViewItem& option, const QModelIndex& index ) const
{
	QStyleOptionViewItem opt = option;

	samplv1widget_programs *pWidget
		= qobject_cast<samplv1widget_programs *>(parent());
	if (pWidget) {
	...
	const QStyleOptionViewItem& /*option*/, const QModelIndex& index ) const
{
	...
	{
		if (index.parent().isValid()) {
			samplv1widget_presets::ComboBox *pComboBox
				= new samplv1widget_presets::ComboBox(pParent);
	...
	QWidget *pEditor, const QModelIndex& index ) const
{
	...
	case 1: // Text.
	{
		...
		if (index.parent().isValid()) {
			samplv1widget_presets::ComboBox *pComboBox
				= qobject_cast<samplv1widget_presets::ComboBox *> (pEditor);
			if (pComboBox) {
...
void samplv1widget_programs::addPresetItems ( samplv1_presets *pPresets )
{
	...
	while (preset_iter.hasNext()) {
		...
		samplv1_presets::Bank *pBank = pPresets->find_preset_bank(sPreset);
