...
//
int samplv1_config::loadPresetsConf (
	QSettings *pSettings, samplv1_presets *pPresets )
{
	int nconfs = 0;
...
int samplv1_config::loadPresetsConfDir (
	samplv1_presets *pPresets, const QDir& dir, QStringList& confs )
{
	int nconfs = 0;
	...
	while (iter.hasNext()) {
		...
		if (!confs.contains(sFilename)) {
			samplv1_config::importPresets(sFilename, pPresets);
