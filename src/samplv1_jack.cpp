...
class samplv1_alsa_thread : public QThread
{
public:

	samplv1_alsa_thread(samplv1_jack *sampl)
...
static void samplv1_jack_on_shutdown ( void *arg )
{
	static_cast<samplv1_jack *> (arg)->shutdown();
}
...
//----------------------------------------------------------------------
// samplv1_jack_session_event -- JACK session event callabck
//

static void samplv1_jack_session_event (
	jack_session_event_t *pSessionEvent, void *pvArg )
{
	samplv1_jack *pSampl = static_cast<samplv1_jack *> (pvArg);

	if (pSampl)
		pSampl->sessionEvent(pSessionEvent);
}
...
samplv1_jack::samplv1_jack (const char *client_name) : samplv1(2)
{
	...
	::memset(m_params, 0, samplv1::NUM_PARAMS * sizeof(float));
...
jack_client_t *samplv1_jack::client (void) const
{
	return m_client;
}
...
void samplv1_jack::deactivate (void)
{
	if (m_activated) {
		if (m_client) {
			m_activated = false;
...
snd_seq_t *samplv1_jack::alsa_seq (void) const
{
	return m_alsa_seq;
}
...
samplv1_jack_application::samplv1_jack_application ( int& argc, char **argv )
	: QObject(nullptr), m_pApp(nullptr), m_bGui(true),
		m_sClientName(PROJECT_NAME), m_pSampl(nullptr), m_pWidget(nullptr)
	  #ifdef CONFIG_NSM
...
#ifdef CONFIG_DEBUG
	qDebug("samplv1_jack::openSession()");
#endif
...
#ifdef CONFIG_DEBUG
	qDebug("samplv1_jack::showSession()");
#endif
...
#ifdef CONFIG_DEBUG
	qDebug("samplv1_jack::hideSession()");
#endif
...
void samplv1_jack_application::watchdog_slot (void)
{
	if (m_pSampl && m_pSampl->client() == nullptr) {
		const QByteArray aClientName
			= m_sClientName.toLocal8Bit();
		const char *client_name
			= aClientName.constData();
		m_pSampl->open(client_name);
		m_pSampl->activate();
	}
...
// Pseudo-singleton instance.
samplv1_jack_application *samplv1_jack_application::g_pInstance = nullptr;
