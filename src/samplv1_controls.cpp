...
public:
	...
	samplv1_controls::Type type() const
		{ return samplv1_controls::Type(status() & 0x700); }
...
public:
	...
	bool process ( const samplv1_controls::Event& event )
	{
		...
		if (event.key.param == RPN_MSB) {
			...
			if (item.is_any() && item.type() != samplv1_controls::RPN)
				enqueue(item);
			if (item.is_status() // RPN nullptr [MSB]
				&& item.type() == samplv1_controls::RPN && item.is_param_lsb()
		...
		if (event.key.param == RPN_LSB) {
			...
			if (item.is_status() // RPN nullptr [LSB]
				&& item.type() == samplv1_controls::RPN && item.is_param_msb()
		...
		if (event.key.param == NRPN_MSB) {
			xrpn_item& item = get_item(channel);
			if (item.is_any() && item.type() != samplv1_controls::NRPN)
				enqueue(item);
		...
		if (event.key.param > CC14_MSB_MIN && event.key.param < CC14_MSB_MAX) {
			xrpn_item& item = get_item(channel);
			if (item.is_any() && item.type() != samplv1_controls::CC14) {
				enqueue(item);
				item.clear();
				--m_count;
			}
			...
			if ((item.is_param_msb() && item.is_value_msb())
				|| (item.type() == samplv1_controls::CC14 && item.is_param_lsb()
		...
		if (event.key.param > CC14_LSB_MIN && event.key.param < CC14_LSB_MAX) {
			...
			if ((item.is_param_lsb() && item.is_value_lsb())
				|| (item.type() == samplv1_controls::CC14 && item.is_param_msb()
		...
		if (item.type() == samplv1_controls::CC14) {
			...
			//	--m_count;
				...
				const unsigned short status
					= samplv1_controls::CC | item.channel();
				if (item.is_param_msb() && item.is_value_msb())
...
void samplv1_controls::process_event ( const Event& event )
{
	...
	bool bSync = (data.flags & Hook) || !samplv1_param::paramFloat(index);
	if (!bSync)
		bSync = data.sync;
	if (!bSync) {
		const float v0 = data.val;
		const float v1 = samplv1_param::paramScale(index,
	...
	if (bSync) {
		m_sched_out.schedule_event(index,
			samplv1_param::paramValue(index, fScale));
	}
}
