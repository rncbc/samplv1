...
//---------------------------------------------------------------------------
// samplv1_nsm - OSC (liblo) callback methods.
	...
	lo_arg **argv, int /*argc*/, lo_message /*msg*/, void *user_data )
{
	samplv1_nsm *pNsmClient
		= static_cast<samplv1_nsm *> (user_data);
...
//---------------------------------------------------------------------------
// samplv1_nsm - NSM OSC client agent.

// Constructor.
samplv1_nsm::samplv1_nsm (
...
bool samplv1_nsm::is_active (void) const
{
	return m_active;
}
...
const QString& samplv1_nsm::manager (void) const
{
	return m_manager;
}


const QString& samplv1_nsm::capabilities (void) const
{
...
const QString& samplv1_nsm::path_name (void) const
{
	return m_path_name;
}


const QString& samplv1_nsm::display_name (void) const
{
	return m_display_name;
}


const QString& samplv1_nsm::client_name (void) const
{
...
void samplv1_nsm::visible ( bool is_visible )
{
#ifdef CONFIG_LIBLO
	if (m_address && m_server && m_active) {
...
void samplv1_nsm::progress ( float percent )
{
#ifdef CONFIG_LIBLO
	if (m_address && m_server && m_active) {
		lo_send_from(m_address,
...
void samplv1_nsm::message ( int priority, const QString& mesg )
{
#ifdef CONFIG_LIBLO
	if (m_address && m_server && m_active) {
		lo_send_from(m_address,
...
void samplv1_nsm::open_reply ( ReplyCode reply_code )
{
	reply("/nsm/client/open", reply_code);
}


void samplv1_nsm::save_reply ( ReplyCode reply_code )
{
	reply("/nsm/client/save", reply_code);
}


void samplv1_nsm::reply ( const QString& path, ReplyCode reply_code )
{
...
void samplv1_nsm::nsm_announce_error (
	const char *mesg )
{
	m_active = false;
...
#ifdef CONFIG_DEBUG
	qDebug("samplv1_nsm::nsm_announce_error: %s.", mesg);
#endif
}


// Server announce reply.
void samplv1_nsm::nsm_announce_reply (
	const char *mesg,
	const char *manager,
...
#ifdef CONFIG_DEBUG
	qDebug("samplv1_nsm::nsm_announce_reply: %s.", mesg);
#endif
}


// Client open callback.
void samplv1_nsm::nsm_open (
...
void samplv1_nsm::nsm_save (void)
{
#ifdef CONFIG_DEBUG
	qDebug("samplv1_nsm::nsm_save: "
		"path_name=\"%s\" display_name=\"%s\" client_name=\"%s\".",
		m_path_name.toUtf8().constData(),
		m_display_name.toUtf8().constData(),
		m_client_name.toUtf8().constData());
...
void samplv1_nsm::nsm_loaded (void)
{
#ifdef CONFIG_DEBUG
	qDebug("samplv1_nsm::nsm_loaded: "
		"path_name=\"%s\" display_name=\"%s\" client_name=\"%s\".",
		m_path_name.toUtf8().constData(),
		m_display_name.toUtf8().constData(),
		m_client_name.toUtf8().constData());
...
void samplv1_nsm::nsm_show (void)
{
#ifdef CONFIG_DEBUG
	qDebug("samplv1_nsm::nsm_show: "
		"path_name=\"%s\" display_name=\"%s\" client_name=\"%s\".",
		m_path_name.toUtf8().constData(),
		m_display_name.toUtf8().constData(),
		m_client_name.toUtf8().constData());
...
void samplv1_nsm::nsm_hide (void)
{
#ifdef CONFIG_DEBUG
	qDebug("samplv1_nsm::nsm_hide: "
		"path_name=\"%s\" display_name=\"%s\" client_name=\"%s\".",
		m_path_name.toUtf8().constData(),
		m_display_name.toUtf8().constData(),
		m_client_name.toUtf8().constData());
